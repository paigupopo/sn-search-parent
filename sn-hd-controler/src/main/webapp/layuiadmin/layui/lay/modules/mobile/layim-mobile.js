...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function(exports){
  ...
  var touch = function(obj, child, fn){
    var move, type = typeof child === 'function', end = function(e){
      ...
      setTimeout(function(){
        othis.removeAttr('com.data-lock');
  ...
  var listTpl = function(options){
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, com.data){ length++; }}'
      ,'<li layim-event="chat" com.data-type="'+ options.type +'" com.data-index="'+ (options.index ? '{{'+ options.index +'}}' : (options.type === 'history' ? '{{com.data.type}}' : options.type) +'{{com.data.id}}') +'" class="layim-'+ (options.type === 'history' ? '{{com.data.type}}' : options.type) +'{{com.data.id}} {{ com.data.status === "offline" ? "layim-list-gray" : "" }}"><div><img src="{{com.data.avatar}}"></div><span>{{ com.data.username||data.groupname||data.name||"佚名" }}</span><p>{{ com.data.remark||data.sign||"" }}</p><span class="layim-msg-status">new</span></li>'
    ,'{{# }); if(length === 0){ }}'
  ...
  //聊天主模板
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.com.data.type}}">'
    ...
    ,'<div class="layim-chat-footer">'
      ...
      ,'<div class="layim-chat-tool" com.data-json="{{encodeURIComponent(JSON.stringify(d.com.data))}}">'
        ...
        ,'<span class="layui-icon layim-tool-image" title="发送文件" layim-event="image" com.data-type="file">&#xe61d;<input type="file" name="file"></span>'
         ,'{{# }; }}'
  ...
  layui.data.content = function(content){
    ...
    .replace(/audio\[([^\s]+?)\]/g, function(audio){  //转义音频
      return '<div class="layui-unselect layui-layim-audio" layim-event="playAudio" com.data-src="' + audio.replace(/(^audio\[)|(\]$)/g, '') + '"><i class="layui-icon">&#xe652;</i><p>音频消息</p></div>';
    })
    .replace(/video\[([^\s]+?)\]/g, function(video){  //转义音频
      return '<div class="layui-unselect layui-layim-video" layim-event="playVideo" com.data-src="' + video.replace(/(^video\[)|(\]$)/g, '') + '"><i class="layui-icon">&#xe652;</i></div>';
    })
  ...
  var elemChatMain = ['<li class="layim-chat-li{{ d.mine ? " layim-chat-mine" : "" }}">'
    ,'<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>'
      ,'{{ d.username||"佚名" }}'
    ,'</cite></div>'
    ,'<div class="layim-chat-text">{{ layui.com.data.content(d.content||"&nbsp;") }}</div>'
  ,'</li>'].join('');
  ...
  var anim = 'layui-anim-upbit', events = { 
    ...
    chat: function(othis){
      ...
      var list = othis.attr('com.data-list') || othis.index(), data = {};
      if(type === 'friend'){
